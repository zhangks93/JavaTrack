...
class Markdown {
    ...
    private String parseHeader(String markdown) {
        ...
        int counts = (int) markdown.chars().mapToObj(i -> (char)i).filter(item -> item == '#').count();

        System.out.println(count);
        System.out.println(counts);
        return  count == 0 ? null: "<h" + Integer.toString(count) + ">" + markdown.substring(count + 1) + "</h" + Integer.toString(count)+ ">";
    }
